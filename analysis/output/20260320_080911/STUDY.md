...
| Minimum (bottleneck) | 111 (tick 46,000) |
| Final population | 112 |
| Growth rate | 0.3% per 1K ticks (post-bottleneck) |

The initial crash reflects **purifying selection**: cells with random neural networks that fail to photosynthesize or manage energy are eliminated. Only ~11% of initial genomes survive. The survivors then expand as successful strategies reproduce.
...
| Post-crash movement | 3.6% (non-movers survive) |
| Final movement | 0.0% |
| Movement evolution rate | +-0.0092 per 1K ticks |
...
Cells began clustered in the light zone (x < 166) and expanded into the **light zone** (avg x = 139.2). This spatial expansion indicates cells evolved the ability to survive outside the primary energy source, using chemical deposits for sustenance.
...
| Final Shannon index | 4.694 |
| Final unique genomes | 110 |
| Dominant genome fraction | 1.7857% |

Shannon diversity *increased* over the run, indicating the evolution of multiple coexisting strategies rather than a single dominant genome. The dominant genome accounts for only 1.7857% of the population — extreme diversity.
...
| Final avg energy | 25.3 |
| Final avg repmat | 177.3 |
| Max observed age | 2,848 ticks |

Energy accumulation shows cells evolved increasingly efficient metabolic strategies. The max observed age of 2,848 ticks (1x the nominal max age of 5,000) indicates lineages with exceptional survival ability.

### 6. Environmental Pressure

| Metric | Value |
|--------|-------|
| Avg waste at cells | 0.4657 |
| Peak waste | 0.5702 |
| Cells above toxicity | 96.4% |
| Bright zone | 98.2% |
| Dim zone | 0.9% |
| Dark zone | 0.9% |
...
|-------|-----------|-------------|
| Crash | 0 – 46,000 | Population drops from 1015 to 111 (89% mortality) |
| Waste Pressure | 7,000 – 49,000 | >5% of cells above toxicity threshold from tick 7000, peak 98.3% |
| Bonding Emergence | 2,000 – 49,000 | Bond fraction exceeds 1% at tick 2000, reaches 5.4% by end |
