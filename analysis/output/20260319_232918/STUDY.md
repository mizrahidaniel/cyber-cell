...
This simulation run achieved: **chemotaxis (25% of cells moving), long-lived lineages (max age 36,601 ticks)**.
...
| Minimum (bottleneck) | 376 (tick 6,000) |
| Final population | 461 |
| Growth rate | 0.5% per 1K ticks (post-bottleneck) |
...
| Post-crash movement | 25.0% (non-movers survive) |
| Final movement | 25.2% |
...
2. **Crash phase**: Movement drops to ~25% — stationary photosynthesizers survive the bottleneck
3. **Evolution phase**: Movement rises to 25% — but this time it is *directed*, not random
...
| Final avg X | 125.8 |
| Expansion rate | 0.3 units per 1K ticks |

Cells began clustered in the light zone (x < 166) and expanded into the **light zone** (avg x = 125.8). This spatial expansion indicates cells evolved the ability to survive outside the primary energy source, using chemical deposits for sustenance.
...
| Final Shannon index | 6.133 |
| Final unique genomes | 461 |
| Dominant genome fraction | 0.2169% |

Shannon diversity *increased* over the run, indicating the evolution of multiple coexisting strategies rather than a single dominant genome. The dominant genome accounts for only 0.2169% of the population — extreme diversity.
...
| Final avg energy | 188.5 |
| Final avg repmat | 656.9 |
| Max observed age | 36,601 ticks |

Energy accumulation shows cells evolved increasingly efficient metabolic strategies. The max observed age of 36,601 ticks (7x the nominal max age of 5,000) indicates lineages with exceptional survival ability.

## Evolutionary Phases Detected

| Phase | Tick Range | Description |
|-------|-----------|-------------|
| Crash | 0 – 6,000 | Population drops from 2000 to 376 (81% mortality) |
| Chemotaxis Emergence | 2,000 – 49,000 | Movement fraction exceeds 15% at tick 2000, reaches 25.2% by end |
| Bonding Emergence | 2,000 – 49,000 | Bond fraction exceeds 1% at tick 2000, reaches 10.2% by end |
