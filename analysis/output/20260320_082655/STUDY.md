...
This simulation run achieved: **cross-zone exploration (avg position x=255)**.
...
| Minimum (bottleneck) | 52 (tick 7,000) |
| Final population | 292 |
| Growth rate | 0.9% per 1K ticks (post-bottleneck) |

The initial crash reflects **purifying selection**: cells with random neural networks that fail to photosynthesize or manage energy are eliminated. Only ~5% of initial genomes survive. The survivors then expand as successful strategies reproduce.
...
| Post-crash movement | 1.9% (non-movers survive) |
| Final movement | 0.0% |
| Movement evolution rate | +0.0000 per 1K ticks |
...
| Final avg X | 254.6 |
| Expansion rate | 0.9 units per 1K ticks |

Cells began clustered in the light zone (x < 166) and expanded into the **dim zone** (avg x = 254.6). This spatial expansion indicates cells evolved the ability to survive outside the primary energy source, using chemical deposits for sustenance.
...
| Final Shannon index | 5.568 |
| Final unique genomes | 272 |
| Dominant genome fraction | 1.3699% |

Shannon diversity *increased* over the run, indicating the evolution of multiple coexisting strategies rather than a single dominant genome. The dominant genome accounts for only 1.3699% of the population — extreme diversity.
...
| Final avg energy | 56.7 |
| Final avg repmat | 182.5 |
| Max observed age | 5,000 ticks |

Energy accumulation shows cells evolved increasingly efficient metabolic strategies. The max observed age of 5,000 ticks (1x the nominal max age of 5,000) indicates lineages with exceptional survival ability.

### 6. Environmental Pressure

| Metric | Value |
|--------|-------|
| Avg waste at cells | 0.0682 |
| Peak waste | 0.5782 |
...
|-------|-----------|-------------|
| Crash | 0 – 7,000 | Population drops from 1015 to 52 (95% mortality) |
| Expansion | 7,000 – 9,000 | Population doubles to 149 |
| Zone Exploration | 60,000 – 200,000 | Average cell position crosses into dim zone (x>166) at tick 60000, reaches x=255 |
| Waste Pressure | 6,000 – 200,000 | >5% of cells above toxicity threshold from tick 6000, peak 62.8% |
| Bonding Emergence | 2,000 – 200,000 | Bond fraction exceeds 1% at tick 2000, reaches 9.9% by end |
