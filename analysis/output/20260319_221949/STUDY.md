...
This simulation run achieved: **stable self-sustaining population, chemotaxis (29% of cells moving)**.
...
| Minimum (bottleneck) | 631 (tick 1,000) |
| Final population | 1,155 |
| Growth rate | 1.3% per 1K ticks (post-bottleneck) |

The initial crash reflects **purifying selection**: cells with random neural networks that fail to photosynthesize or manage energy are eliminated. Only ~32% of initial genomes survive. The survivors then expand as successful strategies reproduce.
...
| Post-crash movement | 22.2% (non-movers survive) |
| Final movement | 29.3% |
| Movement evolution rate | +0.0017 per 1K ticks |

Movement follows a characteristic **U-shaped curve**:
1. **Random phase**: Initial genomes produce ~26% movement (noise)
2. **Crash phase**: Movement drops to ~22% — stationary photosynthesizers survive the bottleneck
3. **Evolution phase**: Movement rises to 29% — but this time it is *directed*, not random
...
Cells began clustered in the light zone (x < 166) and expanded into the **light zone** (avg x = 96.7). This spatial expansion indicates cells evolved the ability to survive outside the primary energy source, using chemical deposits for sustenance.
...
| Final Shannon index | 7.052 |
| Final unique genomes | 1,155 |
| Dominant genome fraction | 0.0866% |

Shannon diversity *increased* over the run, indicating the evolution of multiple coexisting strategies rather than a single dominant genome. The dominant genome accounts for only 0.0866% of the population — extreme diversity.
...
| Final avg energy | 137.9 |
| Final avg repmat | 351.9 |
| Max observed age | 5,093 ticks |

Energy accumulation shows cells evolved increasingly efficient metabolic strategies. The max observed age of 5,093 ticks (1x the nominal max age of 5,000) indicates lineages with exceptional survival ability.

## Evolutionary Phases Detected

| Phase | Tick Range | Description |
|-------|-----------|-------------|
| Crash | 0 – 1,000 | Population drops from 2000 to 631 (68% mortality) |
| Chemotaxis Emergence | 2,000 – 49,000 | Movement fraction exceeds 15% at tick 2000, reaches 29.4% by end |
| Bonding Emergence | 2,000 – 49,000 | Bond fraction exceeds 1% at tick 2000, reaches 10.7% by end |
