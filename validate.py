...
Usage:
    python validate.py [--ticks N] [--genome neural|crn] [--no-archipelago]
"""
...
def check_results(results: dict) -> list[tuple[str, bool, str]]:
    ...
    if len(strengths) > 10:
        ...
        # Bonds should NOT all be at initial strength (0.5) — decay is working
        has_variety = strengths_arr.std() > 0.01
        not_all_max = mean_str < 0.95
...
if __name__ == "__main__":
    ...
    if not args.no_plot:
        ...
        out_dir = os.path.join("analysis", "output",
                               f"validate_{args.genome}_{args.ticks}t_{timestamp}")
